...
int main() {
	...
	while (1) {
		...
		values[position] += (initial_value + PADDING_VALUE);
		if (initial_value < THRESHOLD_VALUE) {         								// -- WHITE 
			printf("\tWHITE 00-> %0.2f\n", initial_value + PADDING_VALUE);
			while(1){
				result = adc_read(); 												// -- RAW VALUE: 0xfe5
